...
setInterval(()=>{
	const bg_group = document.querySelectorAll('.bg_group > div');
	const s_bg = document.querySelector(`.s${cnt+1}_bg`);

	bg_group.forEach((el)=>{el.style.opacity ='0';
});
s_bg.style.opacity = '1';
if (cnt > 1) {
	cnt = 0;
} else {
	cnt += 1;
...
setInterval(()=>{
	const img_group = document.querySelectorAll('.img_group > div');
	const s_img = document.querySelector(`.s${count+1}_img`);

	img_group.forEach((el)=>{el.style.opacity ='0';
});
s_img.style.opacity = '1';
if (count > 2) {
	count = 0;
} else {
	count += 1;
